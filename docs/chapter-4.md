...
**RF01:** El sistema deberá permitir el registro de usuarios y autenticación con correo y contraseña, asignando roles por organización (administrador, operador, auditor) y aplicando control de acceso por rol.


**RF02:** El sistema deberá gestionar membresías usuario–organización, habilitando alta, baja y cambio de rol con registro de quién y cuándo realizó la acción.


**RF03:** El sistema deberá crear lotes (batch) con datos mínimos obligatorios: nombre de lote, productor/organización, variedad, fecha de cosecha y estado inicial “OPEN”.


**RF04:** El sistema deberá permitir actualizar los datos de un lote mientras esté “OPEN” y bloquear ediciones cuando pase a “CLOSED”.


**RF05:** El sistema deberá generar para cada lote un identificador QR y un payload firmado que permita verificar la autenticidad del lote al consultarlo.


**RF06:** El sistema deberá listar los lotes del productor con filtros por estado, rango de fechas y búsqueda por texto.


**RF07:** El sistema deberá ejecutar el cierre de lote (“CLOSED”) validando reglas de negocio y registrando la marca de tiempo y el usuario responsable.


**RF08:** El sistema deberá registrar eventos de trazabilidad para un lote con los campos: tipo de evento, fecha y hora, actor (usuario), ubicación (location_label, location_lat, location_lon) y observaciones opcionales.
...
**RNF01:** Disponibilidad de lectura pública (consulta por QR) ≥ 99.9% mensual; tiempo medio de recuperación (MTTR) ≤ 15 minutos; seguimiento mediante SLO con política de error budget documentada.


**RNF02:** Disponibilidad de APIs internas (IAM, Batch, Traceability) ≥ 99.5% mensual; MTTR ≤ 30 minutos; ventanas de mantenimiento notificadas con al menos 24 horas de anticipación.


**RNF03:** Rendimiento de APIs internas: latencia P95 ≤ 500 ms y P99 ≤ 1500 ms para operaciones de lectura/escritura de lotes y eventos, medido continuamente con SLIs.


**RNF04:** Rendimiento de la consulta pública por QR: tiempo de primera vista P95 ≤ 800 ms y P99 ≤ 2000 ms, empleando carga progresiva del historial.


**RNF05:** Integridad de trazabilidad: 100% de eventos generan hash inmutable al persistir; ≥ 99% de eventos alcanzan estado CONFIRMED en blockchain en ≤ 5 minutos; el backlog tras una caída se vacía en ≤ 30 minutos.


**RNF06:** Seguridad de credenciales: contraseñas almacenadas con Argon2id cumpliendo parámetros mínimos (memoria ≥ 19 MiB, iteraciones ≥ 2, paralelismo = 1) o equivalente seguro; sal aleatoria única por usuario.


**RNF07:** Autenticación y transporte: todas las APIs expuestas únicamente por HTTPS; tokens de acceso firmados y con expiración; rotación y revocación de claves documentadas; sin protocolos inseguros habilitados.


**RNF08:** Autorización de mínimo privilegio: credenciales de servicios acotadas a su alcance (p. ej., el worker solo puede actualizar blockchain_status/transaction_hash); 0 operaciones fuera de alcance en auditorías trimestrales.


**RNF09:** Auditabilidad: 100% de acciones críticas generan entradas de auditoría inmutables; retención de logs ≥ 365 días; posibilidad de exportación; trazas correlacionables por ID de solicitud.


**RNF10:** Observabilidad: definición de SLIs (disponibilidad, latencia, tasa de errores) y alertas por consumo de error budget; tableros en tiempo real para producto y operaciones.


**RNF11:** Calidad de producto: evidencias de prueba que demuestren confiabilidad, seguridad y mantenibilidad (ISO/IEC 25010); criterios y métricas acordados para cada característica antes de la liberación.


**RNF12:** Escalabilidad: mantener objetivos de latencia P95 al multiplicar por ×10 el volumen base de eventos en pruebas de carga; utilización de CPU por instancia ≤ 70%; 0 pérdida de mensajes.


**RNF13:** Idempotencia: operaciones de registro de eventos y anclaje deben ser idempotentes; tasa máxima de duplicados aceptada ≤ 0.1% en pruebas de reintento controlado.


**RNF14:** Portabilidad y despliegue: servicios empaquetados en contenedores; soporte para Linux x86_64 y ARM64 sin cambios de código; tiempo máximo de despliegue o rollback ≤ 15 minutos.


**RNF15:** Documentación técnica: cobertura OpenAPI del 100% de endpoints públicos; diagramas C4 y ER actualizados en cada versión; guía de versionado y deprecación publicada.


**RNF16:** Protección de documentos: 100% de certificados o evidencias con doc_sha256 no nulo cuando exista doc_url; discrepancia de hash genera alerta y bloqueo de descarga hasta su resolución.
...
Miro: https://miro.com/welcomeonboard/SlZKNUJTTEJBcTZRMGNDYmFqOXR4OEh6aTlMTE83amYxVjRlMUQxTWRUdE56QkplZUpUOHp6V1NhNkUxU04xSjNzSXpNL1VUUzl5NUUxT1dqTTBITW5uZW9sQlAvMVluZDlJd0NHZ0U5SDlzcWUvSnUvYWdKdUZ4dWRzOFY1c2tzVXVvMm53MW9OWFg5bkJoVXZxdFhRPT0hdjE=?share_link_id=353989808784
...
Miro: https://miro.com/welcomeonboard/ZTZMcy9zWURYSjNrZ1FSK3lnRjZLZU1OUzdIT2VrT3VFN21RSGc2ZEtFeExHNWVCRlBoaEZQSjBPTkJUN09NTmt4VEQ4WVdBUEFJY2tkenNRTGhlbkhuZW9sQlAvMVluZDlJd0NHZ0U5SDgzRDBxSUNRcWF4RXlCU1FtcktJSGtQdGo1ZEV3bUdPQWRZUHQzSGl6V2NBPT0hdjE=?share_link_id=605140937332
...
<img src="../assets/img/chapter-4/DiagramContx.png" alt="Context Mapping" style="width:50%; height:auto;">
